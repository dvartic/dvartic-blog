import { Button, Heading, VStack, Tooltip, Box, useColorModeValue, useDisclosure, Collapse, useToast } from '@chakra-ui/react';
...
export function QNA({strippedContent}: Props) {
    ...
    const buttonElementsMemo = useMemo(buttonElements, [model, isOpen]);

    return (
        <Box w='100%'>
            <Box display='flex' w='100%' h={0} position='relative' top={1} right={1}>
                <Box h={0} ml='auto'>
                    <Tooltip label='Load a TensorFlow AI/ML Model trained to answer questions in natural language.' fontSize='md' hasArrow>
                        <InfoIcon />
                    </Tooltip>
                </Box>
            </Box>
            <VStack w='100%' bg={bgColor} borderTop='2px' borderBottom='2px' borderColor='black' pt={2} pb={2}>
                <Heading mr={8} ml={8} textAlign='center' as='h3' size='lg' color={textColor}>Let an AI answer your questions</Heading>
                <Button colorScheme='pink' rightIcon={buttonElementsMemo?.icon} onClick={loadModel} isLoading={isModelLoading} loadingText='Loading'>{buttonElementsMemo?.text}</Button>
