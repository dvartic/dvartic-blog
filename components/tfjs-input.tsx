    ...
    return (
        <VStack spacing={5}>
            <Flex w='100%' justifyContent='space-around' alignItems='center'>
                ...
                <Button colorScheme='pink' rightIcon={<MdBolt />} isLoading={isModelExecuting} loadingText='Executing' onClick={handleClick}>
                    Run
                </Button>
            </Flex>
            <Box bg={fieldsBg} w='100%' p={4} borderRadius={6} border='1px' borderColor={borderColor} minH={16}>
                {answers &&
                    <TfjsResultsMotion answers={answers} hasSuccessful={hasSuccessful} initial={false} animate={controls} />
                }
            </Box>
        </VStack>
    )
